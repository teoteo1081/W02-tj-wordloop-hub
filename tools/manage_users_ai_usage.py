"""
================================================================================
 manage_users_ai_usage.py — QUẢN LÝ NGƯỜI HỌC + THEO DÕI CHI PHÍ AI
 (TJ WordLoop Hub, chế độ Cloud)
================================================================================
 BẢN 2026-09-14 — NHÂN BẢN từ tools/manage_users.py (giữ file gốc nguyên vẹn,
 KHÔNG sửa trực tiếp - xem lịch sử/ghi chú đầy đủ ở đó), CHỈ THÊM cột thống
 kê dùng AI (theo yêu cầu Thao "tracking performance và quản lý cost"):
   - Quota AI hôm nay: đã dùng bao nhiêu/3 Block (giới hạn user thường,
     xem supabase/functions/openai-proxy|gemini-proxy/index.ts checkQuota) -
     Admin hiện "👑 Không giới hạn".
   - Tổng token AI hôm nay / cộng dồn TỪ TRƯỚC ĐẾN GIỜ (prompt+completion),
     đọc từ bảng ai_usage_daily.{prompt,completion,total}_tokens - CHỈ có
     dữ liệu cho các lượt gọi SAU KHI đã chạy migrate 2026-09-14 (4 cột
     token + bảng site_visits trong tools/supabase_schema.sql) VÀ deploy
     lại 2 Edge Function - lượt gọi CŨ hơn không có số token (hiện 0).
   - Ước tính chi phí AI (USD): CHỈ tính phần OpenAI (Gemini free tier =
     0đ) - dùng bảng giá gpt-4o-mini (model mặc định của proxy, xem
     PRICING_USD bên dưới, PHẢI khớp tay với Context.OPENAI_PRICING trong
     js/context.js - không có gì tự đồng bộ 2 nơi này) - SAI LỆCH nếu 1
     lượt nào đó gọi model khác (proxy không lưu lại model đã dùng mỗi
     dòng, chỉ có tổng token).
   - Lượt vào app hôm nay / 7 ngày qua: đọc bảng site_visits (ghi bởi
     js/auth.js mỗi lần đăng nhập Cloud thành công) - CHỈ có dữ liệu SAU
     KHI đã chạy migrate 2026-09-14, không suy ra được lượt truy cập cũ.

 Các cột/sheet còn lại (Chi tiết Từ/Chi tiết Block/tạo NEW_USERS...) giữ
 NGUYÊN logic gốc - xem docstring đầy đủ trong tools/manage_users.py.
 ...
 CÁCH DÙNG:
   ...
   2. Đã chạy migrate 2026-09-14 trong tools/supabase_schema.sql (4 cột
      token trên ai_usage_daily + bảng site_visits) VÀ deploy lại
      openai-proxy/gemini-proxy - thiếu bước này cột AI/lượt truy cập vẫn
      hiện nhưng toàn số 0 (không lỗi, chỉ không có dữ liệu).
   3. Sửa NEW_USERS bên dưới nếu muốn thêm người học mới.
   4. Chạy: python tools/manage_users_ai_usage.py
   5. Mở file Excel vừa xuất (đường dẫn in ra cuối cùng) để lấy link gửi
      cho từng người.
================================================================================
"""
...
OUTPUT_XLSX = OUTPUT_DIR / f"nguoi_hoc_ai_usage_{datetime.datetime.now():%Y%m%d_%H%M%S}.xlsx"

# Bảng giá OpenAI (USD / 1 triệu token) - PHẢI khớp tay với Context.OPENAI_PRICING
# trong js/context.js (không có gì tự đồng bộ 2 nơi) - chỉ dùng để ƯỚC TÍNH
# chi phí ai_usage_daily.provider=="openai" (Gemini free tier = luôn 0đ).
# Model không xác định được (proxy không lưu lại mỗi dòng) -> giả định
# "gpt-4o-mini" (model mặc định của openai-proxy/index.ts).
PRICING_USD = {
...
DEFAULT_MODEL_FOR_COST = "gpt-4o-mini"
NON_ADMIN_DAILY_BLOCK_CAP = 3  # PHẢI khớp NON_ADMIN_DAILY_BLOCK_CAP trong 2 Edge Function proxy
...
def _today_key():
    return datetime.datetime.utcnow().strftime("%Y-%m-%d")  # khớp "giờ UTC server" mà 2 proxy/js/auth.js dùng


def _fetch_ai_usage():
    """ai_usage_daily - CHỈ đọc được các cột token mới nếu đã chạy migrate
    2026-09-14 (xem tools/supabase_schema.sql) - fallback đọc KHÔNG kèm
    token (coi như 0) thay vì crash cả script nếu chưa migrate."""
    try:
        return fetch_all(
            "ai_usage_daily",
            "user_id,block_id,date_key,provider,call_count,prompt_tokens,completion_tokens,total_tokens")
    except requests.HTTPError:
        print("  ⚠️ Chưa có cột token trên bảng ai_usage_daily (chạy SQL migrate 2026-09-14 trước) - "
              "cột token/chi phí AI trong Excel sẽ hiện toàn 0.")
        try:
            rows = fetch_all("ai_usage_daily", "user_id,block_id,date_key,provider")
            for r in rows:
                r.setdefault("call_count", 1)
                r.setdefault("prompt_tokens", 0)
                r.setdefault("completion_tokens", 0)
...
def _fetch_site_visits():
    """site_visits - bảng MỚI (2026-09-14), có thể chưa tồn tại trên Supabase
    nếu chưa chạy migrate -> fallback rỗng (cột lượt truy cập hiện toàn 0)
    thay vì crash cả script."""
    try:
        return fetch_all("site_visits", "user_id,date_key,visit_count")
    except requests.HTTPError:
        print("  ⚠️ Bảng 'site_visits' chưa tồn tại trên Supabase (chạy SQL migrate 2026-09-14 trước) - "
              "cột lượt truy cập trong Excel sẽ hiện toàn 0.")
        return []


def _openai_cost_usd(prompt_tokens, completion_tokens):
    price = PRICING_USD.get(DEFAULT_MODEL_FOR_COST)
    return (prompt_tokens or 0) / 1e6 * price["in"] + (completion_tokens or 0) / 1e6 * price["out"]


def _ai_usage_by_user(ai_rows):
    """Gom ai_usage_daily theo user_id -> {blocks_today, tokens_today,
    tokens_alltime, cost_alltime_usd, calls_alltime}. 'hôm nay' so theo
    _today_key() (giờ UTC, khớp date_key server ghi) - CHẠY SCRIPT GẦN NỬA
    ĐÊM UTC CÓ THỂ LỆCH so với "hôm nay" giờ Việt Nam, chấp nhận được vì
    quota/token cũng tính theo đúng mốc UTC này (xem checkQuota() trong 2
    Edge Function)."""
    today = _today_key()
    out = {}
    for r in ai_rows:
        ...
        agg = out.setdefault(uid, {
            "blocks_today": set(), "tokens_today": 0, "tokens_alltime": 0,
            "cost_alltime_usd": 0.0, "calls_alltime": 0,
        })
        total_tok = r.get("total_tokens") or 0
        agg["tokens_alltime"] += total_tok
        agg["calls_alltime"] += r.get("call_count") or 1
        if (r.get("provider") or "") == "openai":
            agg["cost_alltime_usd"] += _openai_cost_usd(r.get("prompt_tokens"), r.get("completion_tokens"))
        if r.get("date_key") == today:
            agg["blocks_today"].add(r.get("block_id"))
            agg["tokens_today"] += total_tok
    return out


def _visits_by_user(visit_rows):
    """Gom site_visits theo user_id -> {today, last_7d} - '7 ngày qua' gồm
    hôm nay + 6 ngày trước (so chuỗi "YYYY-MM-DD", không cần parse ngày vì
    date_key luôn đúng định dạng này)."""
    today_dt = datetime.datetime.utcnow().date()
    last7_keys = {(today_dt - datetime.timedelta(days=i)).strftime("%Y-%m-%d") for i in range(7)}
    today = _today_key()
    out = {}
    for r in visit_rows:
        uid = r.get("user_id")
        if not uid:
            continue
        agg = out.setdefault(uid, {"today": 0, "last_7d": 0})
        cnt = r.get("visit_count") or 0
        if r.get("date_key") == today:
            agg["today"] += cnt
        if r.get("date_key") in last7_keys:
            agg["last_7d"] += cnt
    return out
...
def build_stats():
    ...
    ai_by_user = _ai_usage_by_user(_fetch_ai_usage())
    visits_by_user = _visits_by_user(_fetch_site_visits())
    ...
    for p in profiles:
        ...
        ai = ai_by_user.get(uid, {"blocks_today": set(), "tokens_today": 0, "tokens_alltime": 0, "cost_alltime_usd": 0.0})
        vis = visits_by_user.get(uid, {"today": 0, "last_7d": 0})
        stats_by_user[uid] = {
            "profile": p,
            "words_mastered": 0,
            "blocks_passed": 0,
            ...
            "overdue": 0,
            # Thống kê AI/lượt truy cập - thêm 2026-09-14, xem docstring đầu file.
            "ai_admin": is_admin,
            "ai_blocks_today": len(ai["blocks_today"]),
            "ai_tokens_today": ai["tokens_today"],
            "ai_tokens_alltime": ai["tokens_alltime"],
            "ai_cost_alltime_usd": ai["cost_alltime_usd"],
            "visits_today": vis["today"],
            "visits_7d": vis["last_7d"],
        }
...
COMMENT_AUTHOR = "manage_users_ai_usage.py"
...
def write_excel(stats_list, total_words, total_blocks, word_rows, block_rows):
    ...
    headers = [
        ...
        "🤖 Quota AI\nhôm nay", "Token AI\nhôm nay", "Token AI\ncộng dồn",
        "💰 Chi phí AI\nước tính (USD)", "📡 Lượt vào app\nhôm nay", "📡 Lượt vào app\n(7 ngày)",
    ]
    notes = {
        ...
        15: f"Số Block KHÁC NHAU người này đã nhờ AI viết bài hôm nay (giờ UTC server) / giới hạn "
            f"{NON_ADMIN_DAILY_BLOCK_CAP} cho user thường - xem checkQuota() trong "
            f"openai-proxy|gemini-proxy/index.ts. 👑 Admin không bị giới hạn.\n\n"
            f"CHỈ có dữ liệu SAU KHI đã chạy migrate 2026-09-14 (cột token trên ai_usage_daily) "
            f"và deploy lại 2 Edge Function - trước đó hiện 0 dù thật ra có dùng.",
        16: "Tổng token AI (prompt+completion cộng lại) người này đã dùng HÔM NAY, cộng cả Gemini "
            "lẫn OpenAI. Đọc từ ai_usage_daily.total_tokens - CHỈ ghi nhận được SAU migrate 2026-09-14.",
        17: "Tổng token AI CỘNG DỒN TỪ TRƯỚC ĐẾN GIỜ (từ lúc migrate 2026-09-14 chạy, không suy "
            "ra được lượt dùng trước đó) - dùng để nhìn tổng khối lượng dùng AI theo thời gian, "
            "không riêng hôm nay.",
        18: f"Ước tính USD đã tốn CỘNG DỒN - CHỈ tính phần gọi OpenAI (provider='openai' trong "
            f"ai_usage_daily, Gemini free tier = luôn 0đ) - dùng bảng giá model "
            f"'{DEFAULT_MODEL_FOR_COST}' (mặc định của openai-proxy, KHÔNG BIẾT model thật đã "
            f"dùng từng lượt vì proxy không lưu lại) - CÓ THỂ SAI LỆCH nếu từng đổi model, chỉ "
            f"nên dùng để ước lượng tương đối, không phải số chính xác 100%.",
        19: "Số lần MỞ APP (đăng nhập Cloud thành công) hôm nay - đọc từ bảng site_visits (ghi bởi "
            "js/auth.js). CHỈ có dữ liệu SAU KHI đã chạy migrate 2026-09-14.",
        20: "Tổng số lần mở app trong 7 ngày gần nhất (tính cả hôm nay) - để nhìn ai còn hoạt động "
            "đều, ai đã lâu không vào lại.",
    }
    ...
    for i, s in enumerate(stats_list):
        ...
        # Cột 15-20: thống kê AI/lượt truy cập (mới 2026-09-14, xem docstring đầu file).
        if s["ai_admin"]:
            c_quota = ws.cell(row=r_idx, column=15, value="👑 Không giới hạn")
            c_quota.font = Font(name="Arial", bold=True, color="B8860B")
        else:
            c_quota = ws.cell(row=r_idx, column=15, value=f"{s['ai_blocks_today']}/{NON_ADMIN_DAILY_BLOCK_CAP}")
            if s["ai_blocks_today"] >= NON_ADMIN_DAILY_BLOCK_CAP:
                c_quota.fill = OVERDUE_FILL
                c_quota.font = OVERDUE_FONT

        ws.cell(row=r_idx, column=16, value=s["ai_tokens_today"])
        ws.cell(row=r_idx, column=17, value=s["ai_tokens_alltime"])
        ws.cell(row=r_idx, column=18, value=round(s["ai_cost_alltime_usd"], 4))
        ws.cell(row=r_idx, column=19, value=s["visits_today"])
        ws.cell(row=r_idx, column=20, value=s["visits_7d"])
        if s["visits_7d"] == 0:
            ws.cell(row=r_idx, column=20).font = DIM_FONT

    widths = [4, 20, 11, 60, 11, 13, 15, 15, 12, 11, 11, 13, 15, 12, 12, 11, 12, 14, 12, 12]
    ...
    notes_txt = [
        ...
        "",
        f"• '🤖 Quota AI hôm nay': số Block KHÁC NHAU đã nhờ AI viết bài hôm nay / giới hạn {NON_ADMIN_DAILY_BLOCK_CAP} cho user thường (👑 Admin không giới hạn) - đỏ = đã dùng hết hôm nay.",
        "• 'Token AI hôm nay' / 'Token AI cộng dồn': tổng token (prompt+completion) đã dùng, cộng cả Gemini lẫn OpenAI.",
        f"• '💰 Chi phí AI ước tính': CHỈ tính phần OpenAI (Gemini free = 0đ), dùng bảng giá model '{DEFAULT_MODEL_FOR_COST}' (mặc định của proxy) - ƯỚC TÍNH TƯƠNG ĐỐI, không phải số chính xác 100% (proxy không lưu model thật đã dùng từng lượt).",
        "• '📡 Lượt vào app': số lần đăng nhập Cloud thành công hôm nay / 7 ngày qua - để biết ai còn hoạt động đều, ai đã lâu không quay lại.",
        "• 4 cột AI/lượt truy cập trên CHỈ có dữ liệu SAU KHI đã chạy migrate 2026-09-14 (tools/supabase_schema.sql: cột token trên ai_usage_daily + bảng site_visits) VÀ deploy lại openai-proxy/gemini-proxy - trước đó hiện toàn 0 dù thật ra có dùng.",
        "",
